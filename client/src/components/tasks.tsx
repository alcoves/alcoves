...
function timeAgo(date: number) {
	...
	if (diff.days > 0) {
		...
		output = `${diff.hours.toFixed(1)} hour${diff.hours !== 1 ? "s" : ""} ago`;
							...
							<Card bg={cardBg}>
								<CardBody>
									<Box>
										<Heading size="md">
											{`${
												task.queueQualifiedName.split("bull:")[1]
											}:${task.name}:${task.id}`}
										</Heading>
									</Box>
									...
									<Tabs py="2">
										...
										<TabPanels>
											...
											<TabPanel
												...
												fontFamily="monospace"
											>
												<p>{JSON.stringify(task.returnvalue, null, 2)}</p>
											</TabPanel>
											<TabPanel
												p="2"
												bg={cardCodeBg}
									...
									<Flex w="100%" justify="space-between">
										<Box>
											<Text fontSize=".75rem">
												{`Started: ${timeAgo(task.timestamp)}`}
											</Text>
											<Text fontSize=".75rem">
										...
										<Flex direction="column" align="end">
											<Text fontSize=".75rem">
												{`ETA: ${task.data.estimatedTimeRemaining ?? "N/A"}`}
											</Text>
