...
module.exports = event => {
  ...
  if (event.req.headers && event.req.headers.cookie) {
    const cookies = event.req.headers.cookie.split(';');

    const { idToken } = cookies.reduce((acc, c) => {
      if (c.includes('idToken=')) acc['idToken'] = c.split('idToken=')[1];
      if (c.includes('accessToken=')) acc['accessToken'] = c.split('accessToken=')[1];
      if (c.includes('refreshToken=')) acc['refreshToken'] = c.split('refreshToken=')[1];
      return acc;
    }, {});
