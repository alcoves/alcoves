...
export default function StudioVideoCard({ v = {}, refetchVideoList }) {
  const { data: video, mutate } = useSWR(`/api/videos/${v?.id}`, fetcher, {
    fallbackData: v,
    refreshInterval: v?.status === 'completed' ? 0 : 2000,
  })
      ...
      <Skeleton isLoaded={Boolean(video.id)}>
        <Box
          ...
          <Flex w='100%' h='100%' justify='center' align='center' direction='column' p='1'>
            <Flex></Flex>
            <Spacer />
            <Flex>{video.status !== 'completed' && <Spinner color='#ffcc00' />}</Flex>
            <Spacer />
        ...
        <Box>
          <Box h='3px'>
            {video.status !== 'completed' && (
              <Progress height='3px' colorScheme='teal' value={video?.percentCompleted} />
            )}
          </Box>
          <Box p='2' bg='transparent' w='100%'>
            <Flex direction='column'>
              <Text isTruncated fontSize='.8rem' fontWeight='700'>
                {video?.title}
              </Text>
              <Metadata v={v} />
            </Flex>
