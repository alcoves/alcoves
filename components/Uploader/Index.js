...
export default function Uploader({ refetch }) {
  ...
  return (
    <>
      <Tooltip label='Upload' openDelay={300} aria-label='upload'>
        <IconButton onClick={onOpen} size='sm' ml='2' icon={<IoVideocam />} />
      </Tooltip>
        ...
        <ModalContent>
          <ModalBody>
            <Flex direction='column'>
              ...
              <Box>
                {uploads.map(item => (
                  <UploadItem item={item} key={item.id} refetch={refetch} dispatch={dispatch} />
                ))}
              </Box>
            </Flex>
          </ModalBody>
