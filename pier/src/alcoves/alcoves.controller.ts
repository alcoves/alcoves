import { Get, Post, Body, Patch, Param, Delete, Controller, NotFoundException } from "@nestjs/common";
...
export class AlcovesController {
  ...
  @CheckAbilities({ action: 'update', subject: 'alcoves' })
	async update(@Param('id') id: string, @Body() updateAlcoveInput: Alcove) {
		...
		const updatedAlcove = await this.alcovesService.update(id, updateAlcoveInput);
		if (updatedAlcove) {
